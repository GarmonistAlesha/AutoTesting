...
public class downloadUploadFile {
    ...
    void xlsTestdownload() throws Exception { //Метод загрузки Excel файла и проверки (необходимо подключить библиотеку
        ...
        //xls.excel.getSheetAt(0)- страница таблицы.getRow(0)- строка таблицы.getCell(0)-столбец таблицы.getStringCellValue()- преобразование в текст содержимого таблицы
...
//      (  testImplementation("com.codeborne:pdf-test:1.5.0") и testImplementation("com.codeborne:xls-test:1.4.3")
    ...
    // в buildgradle com.google.code.gson:gson:2.13.1 maven(найти)
    {
        ...
        try (InputStream is = cl.getResourceAsStream("TestData/testJson.json");
            ...
            Assertions.assertTrue(jsonObject.getAsJsonArray("hobbies").isJsonArray()); //проверка, что hobbies является массивом
    ...
    @Test
    void homeWorkCsv() throws Exception {
        try (InputStream hw = cl.getResourceAsStream("TestData/homeWork.zip");
             ZipInputStream csv = new ZipInputStream(hw)) {
            ZipEntry entry;
            while ((entry = csv.getNextEntry()) != null) //Цикл распоковки файлов с архива( повторяет операцию до тех пор,
            // пока есть файлы)
            {
                if (entry.getName().equals("testCsv1.csv"))
                {
                    CSVReader csvReader = new CSVReader(new InputStreamReader(hw));
                    List<String[]> content = csvReader.readAll();//Каждая строчка в CSV это массив string List - это список строчек
                    Assertions.assertTrue(entry.getName().contains("annual-enterprise-survey-2023-financial-year-provisional.csv"));
                    Assertions.assertArrayEquals(new String[]{"Year", "Industry_aggregation_NZSIOC", "Industry_code_NZSIOC", "Industry_name_NZSIOC","Units"}, content.get(0));
                    Assertions.assertArrayEquals(new String[]{"2023", "ILevel 1", "99999", "All industries,Dollars (millions)","H01"}, content.get(1));
                }
            }
        }
        }
    @Test
    void homeWorkPdf() throws Exception {
        try (InputStream hw = cl.getResourceAsStream("TestData/homeWork.zip");
             ZipInputStream pdf = new ZipInputStream(hw)) {
            ZipEntry entry;
            while ((entry = pdf.getNextEntry()) != null)
            {
                if (entry.getName().equals("Ponomarev Mikhail.pdf"))
                {

                PDF pdf1 = new PDF(pdf);
                Assertions.assertTrue(entry.getName().contains("Ponomarev Mikhail.pdf"));
                pdf1.text.contains("mvponomarevwork@gmail.com");
                }
            }
        }
    }
    @Test
    void homeWorkXls() throws Exception {
        try (InputStream hw = cl.getResourceAsStream("TestData/homeWork.zip");
             ZipInputStream xls = new ZipInputStream(hw)) {
            ZipEntry entry;
            while ((entry = xls.getNextEntry()) != null)
            {
                if (entry.getName().equals("import_ou_xls.xls"))
                {

                    XLS xls1 = new XLS(xls);
                    Assertions.assertTrue(entry.getName().contains("import_ou_xls.xls"));
                    Assertions.assertTrue(xls1.excel.getSheetAt(0).getRow(2).getCell(2).getStringCellValue().startsWith("Маректинг и реклама"));
